    ...
    @if (!Auth::user()->hasRole('Patient'))
        ...
        <div class="form-group col-sm-6 mb-5">
            ...
            <div class="form-check form-switch">
                <input class="form-check-input w-35px h-20px" name="status" type="checkbox" value="1" {{isset($appointment) ? $appointment->is_completed == 0 ? '' : 'checked'  : 'checked'}}>
            </div>
        </div>
        @if (count($customField) >= 0)
            ...
            <div class="form-group col-sm-{{$field['grid']}} mb-5 text-capitalize">
                ...
                @elseif ($field_type == 'multiSelect')
                    {{ Form::select($fieldName.'[]',$field_values, isset($fieldData) ? $fieldData : 0, ['class' => 'form-select', 'placeholder' => $field['field_name'], 'data-control' => 'select2','multiple' => true,$field['is_required'] == 1 ? 'required' : '']) }}
                @else
