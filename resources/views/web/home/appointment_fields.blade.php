    ...
    @if (count($customField) >= 0)
            ...
            <div class="appointment-form__input-block col-sm-{{$field['grid']}} mb-5 text-capitalize">
                ...
                <div class="form-check form-switch">
                    <input style="height: 25px;width:40px" class="form-check-input front-appointment-switch  is-active" name={{$fieldName}} type="checkbox" value="1"
                        tabindex="8" {{ $fieldData == 0 ? '' : 'checked' }}>
                </div>
                @elseif ($field_type == 'select')
                    {{ Form::select($fieldName, $field_values, isset($fieldData) ? $fieldData : null, ['class' => 'selectized','id'=> 'customFieldSelect', 'placeholder' => $field['field_name'], $field['is_required'] == 1 ? 'required' : '']) }}
                @elseif ($field_type == 'multiSelect')
                    {{ Form::select($fieldName . '[]', $field_values, isset($fieldData) ? $fieldData : null, ['class' => 'selectized','id'=> 'customFieldMultiSelect','multiple', 'placeholder' => $field['field_name'], $field['is_required'] == 1 ? 'required' : '']) }}
                @else
