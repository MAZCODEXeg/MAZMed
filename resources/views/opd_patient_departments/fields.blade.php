    ...
    <div class="col-md-2">
        <div class="mb-5">
            <div class="mb-5">
                ...
                {{ Form::number('height', isset($data['last_visit']) ? $data['last_visit']->height : 0, ['placeholder' => __('messages.ipd_patient.height'), 'class' => 'form-control', 'max' => '7', 'step' => '.01']) }}
            </div>
        </div>
    </div>
    <div class="col-md-2">
        ...
        @foreach ($customField as $field)
            ...
            <div class="form-group col-sm-{{$field['grid']}} mb-5 text-capitalize">
                ...
                @if ($field_type == 'date' || $field_type == 'datetime' )
                    {{ Form::text($fieldName, null, ['id'=>$dateType,'class' => (getLoggedInUser()->thememode ? 'bg-light form-control' : 'bg-white form-control'), 'autocomplete' => 'off','placeholder'=>$field['field_name'],$field['is_required'] == 1 ? 'required' : '']) }}
                ...
                @elseif ($field_type == 'select')
                    {{ Form::select($fieldName, $field_values, null, ['class' => 'form-select', 'placeholder' => $field['field_name'], 'data-control' => 'select2',$field['is_required'] == 1 ? 'required' : '' ]) }}
                @elseif ($field_type == 'multiSelect' )
                    {{ Form::select($fieldName.'[]', $field_values, 0, ['class' => 'form-select', 'placeholder' => $field['field_name'], 'data-control' => 'select2','multiple' => true,$field['is_required'] == 1 ? 'required' : '']) }}
                @else
                    {{ Form::$field_type($fieldName, null, ['class' => 'form-control', 'placeholder' => $field['field_name'],$field['is_required'] == 1 ? 'required' : '','rows' => $field_type == 'textarea' ?  4 : '']) }}
                @endif
            </div>
        @endforeach
    @endif
